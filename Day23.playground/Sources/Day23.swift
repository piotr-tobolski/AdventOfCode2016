...
public class Day23 {
    ...
    public func solve() -> (String, String) {
        ...
        let computer = Computer([ "a" : 7 ])
        computer.execute(program)
        print("computer.registers", computer.registers)

        let otherComputer = Computer([ "a" : 12 ])
        otherComputer.execute(program)
        print("otherComputer.registers", otherComputer.registers)

        return (String(computer.registers["a"] ?? Int.min), String(otherComputer.registers["a"] ?? Int.min))
